...
| 文件版本 | v0.3（草稿，待確認） |
| 建立日期 | 2026-09-22 |
| 最後更新 | 2026-09-22（v0.3：參考 Raymond Zeng 財務試算表做法，新增 §2.12 薪酬 / 稅優帳戶 / 長期預測模組與附錄 A；v0.2：硬體環境、AI 開發方式、P-09 商品查找） |
| 狀態 | 🟡 需求討論中 |
| 部署環境 | MacBook Pro（Apple M4）本地端自架、資料不上雲 |
...
5. **私有**：所有資料存放在自家電腦，AI 辨識可切換本地 / 雲端。
...
| **資料私有** | 資料庫、附件皆存本機；外部呼叫（匯率、股價、雲端 LLM）需可在設定中關閉或改用本地方案 |
...
| **可維護性** | 稅率 / 分類 / 規則以設定檔管理；模組化架構便於新增功能 |
...
| 圖表 | **Recharts / ECharts** | Chart.js | ECharts 支援 Sankey、日曆熱力圖 |
...
| **Phase 1** | 核心記帳 | A-01~04、T-01~06、T-09、R-01~04、R-07、E-07、E-08 | Web 手動記帳 + 基本報表 + 淨資產 |
| **Phase 2** | Telegram 快速記帳 | B-01、B-02、B-04、B-05、B-07、N-01、N-07 | 拍收據即記帳 + 每日推播 |
| **Phase 3** | 預算 / 目標 / 訂閱 | G-01~03、G-05、S-01、N-03、N-04 | 預算控管 + 目標反推 + 帳單提醒 |
| **Phase 4** | 投資 | I-01~03、A-06、R-02（含收益） | 持股損益與資產配置 |
| **Phase 5** | 物價追蹤與商品查找 | P-01~04、B-06、P-03、**P-09、B-10、B-11**、P-10 | 收據品項 → 價格趨勢；拍照 / 關鍵字查過往購買金額 |
...
| Q14 | Phase 順序 | 上述 8 個階段順序是否符合你的優先需求？P-09 目前排在 Phase 5，是否要提前？ |
