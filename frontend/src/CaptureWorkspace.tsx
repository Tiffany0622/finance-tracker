...
function DraftEditor({initial,accounts,categories,currency,onClose,onSaved}:{initial:Draft;accounts:Account[];categories:Category[];currency:string;onClose:()=>void;onSaved:()=>void}) {
  ...
  async function reload(){if(dirty&&!window.confirm('重新載入會捨棄尚未儲存的修改，確定繼續？'))return;try{const row=await api<Draft>('/capture/drafts/'+draft.id);setDraft(row);setProposal(row.proposal);setDirty(false);setError('');setAck(false);}catch(e){setError(message(e));}}
  ...
  function close(){if(!busy&&(!dirty||window.confirm('有尚未儲存的修改，確定離開？')))onClose();}
  return <dialog ref={dialogRef} className="finance-dialog capture-editor" aria-labelledby="draft-title" onCancel={e=>{e.preventDefault();close();}}><div className="dialog-heading"><h2 id="draft-title">核對收據草稿</h2><button aria-label="關閉草稿" disabled={busy} onClick={close}><X/></button></div>
    ...
    {!closed&&<Button type="submit" disabled={busy||!dirty}>儲存草稿修改</Button>}</fieldset></form>
    ...
    {!closed&&<><p className="footnote">儲存修改後，再按「確認入帳」。取消草稿會保留照片與歷程。</p><div className="form-actions"><Button variant="secondary" disabled={busy||dirty||draft.status==='processing'} onClick={()=>void action('retry')}>重新辨識</Button><Button disabled={busy||dirty||draft.status==='processing'||draft.warnings.length>0&&!ack} onClick={()=>void action('confirm')}><Check size={16}/>確認入帳</Button></div>{cancelPrompt?<div className="form-hint">確定取消這份草稿？<Button variant="danger" disabled={busy} onClick={()=>void action('cancel')}>確定取消草稿</Button><button onClick={()=>setCancelPrompt(false)}>返回</button></div>:<button className="text-button" disabled={busy} onClick={()=>setCancelPrompt(true)}>取消這份草稿</button>}</>}
    </div></div></dialog>;
}
